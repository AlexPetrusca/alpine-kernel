...
SUITE(pgm)
  ...
  TEST(alloc1)
    void* ptr = malloc(6 * PAGE_SIZE);
    uint64_t base = (((uint64_t) ptr >> 12) + 1) << 12;
    uint64_t handle = pgm_init(base, 4 * PAGE_SIZE);
    ASSERT_EQUAL_INT(pgm_used_pages(handle), 0);
    ASSERT_EQUAL_UINT(pgm_allocate_page(handle), base + 1 * PAGE_SIZE);
    ASSERT_EQUAL_UINT(pgm_allocate_page(handle), base + 2 * PAGE_SIZE);
    ASSERT_EQUAL_UINT(pgm_allocate_page(handle), base + 3 * PAGE_SIZE);
  ...
  TEST(alloc2)
    void* ptr = malloc(103 * PAGE_SIZE);
    uint64_t base = (((uint64_t) ptr >> 12) + 1) << 12;
    uint64_t handle = pgm_init(base, 101 * PAGE_SIZE);
    ASSERT_EQUAL_INT(pgm_used_pages(handle), 0);
    for (uint64_t i = 0; i < 100; i++) {
      uint64_t page = pgm_allocate_page(handle);
      ASSERT_EQUAL_UINT(page, base + (1 + i) * PAGE_SIZE);
    }
  ...
  TEST(alloc_free_1)
    void* ptr = malloc(66 * PAGE_SIZE);
    uint64_t base = (((uint64_t) ptr >> 12) + 1) << 12;
    uint64_t handle = pgm_init(base, 64 * PAGE_SIZE);
    uint64_t p1 = pgm_allocate_page(handle);
    uint64_t p2 = pgm_allocate_page(handle);
    uint64_t p3 = pgm_allocate_page(handle);
    ASSERT_EQUAL_UINT(p1, base + 1 * PAGE_SIZE);
    ASSERT_EQUAL_UINT(p2, base + 2 * PAGE_SIZE);
    ASSERT_EQUAL_UINT(p3, base + 3 * PAGE_SIZE);
  ...
  TEST(alloc_free_2)
    void* ptr = malloc(140 * PAGE_SIZE);
    uint64_t base = (((uint64_t) ptr >> 12) + 1) << 12;
    uint64_t handle = pgm_init(base, 128 * PAGE_SIZE);
    ...
    for (int i = 0; i < 100; i++) {
      uint64_t page = base + (1 + i) * PAGE_SIZE;
