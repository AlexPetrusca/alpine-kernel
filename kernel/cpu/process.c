...
void* pcs_allocate_stack() {
  _pcs_stack_addr += STACK_SIZE;
  return (void*) _pcs_stack_addr;
}

bool pcs_init() {
  uint32_t core_count = cpu_core_count();
  _pcs_stack_addr = mem_get_heap_addr() - core_count * STACK_SIZE;
  try(mem_identity_map_range(_pcs_stack_addr, core_count * STACK_SIZE, MEMORY_STACKS), false, "");
